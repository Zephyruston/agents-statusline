...
| **Quota** | 5-hour and 7-day window usage % (replaces the DeepSeek line for non-deepseek models) |
...
The script itself spawns one `python3` and, for DeepSeek models, one `deepseek status --json` call. Typical render is well under a second; set `timeoutMs` if you want a hard ceiling.
